...
public class RandomMenu {
	...
	public RandomMenu(String userName) {
		...
		chanceLabel.setFont(new Font("楷体", Font.PLAIN, 24));
		chanceLabel.setBounds(10, 180, 400, 40);
		frame.add(chanceLabel);

		// 标签
		JLabel messageLabel = new JLabel("您还有" + count + "次机会！！！");
		messageLabel.setHorizontalAlignment(SwingConstants.CENTER);
		messageLabel.setForeground(Color.black);
		messageLabel.setFont(new Font("楷体", Font.PLAIN, 26));
		messageLabel.setBounds(10, 370, 400, 40);
		...
		ensureButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (Integer.parseInt(number) == randNumber && count >= 1) {
					...
					//如果是新成绩
					int score = count * 2;
					if(ScoreHelper.isNewScore("record_random", userName, score)) {
						//注册成提示界面
                	    Object[] options = {"加入", "不加入"};
                	    int i = JOptionPane.showOptionDialog(null, "新纪录，是否加入排行榜！","加入排行榜?",JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE,null,options,options[0]);
						if(i == 0) {
							//加入排行榜
							ConnectService.updataScore("record_random", userName, score);
					    }
						frame.removeNotify();
						new ChooseMenu(userName);
					}
					else if(!ScoreHelper.isScoreExist("record_random", userName)) {
						// 注册成提示界面
						Object[] options = { "加入", "不加入" };
						...
						if (i == 0) {
							// 加入排行榜
							ConnectService.addScore("record_random", userName, score);
						}
						frame.removeNotify();
						new ChooseMenu(userName);
					}
				...
				} else if (Integer.parseInt(number) > 100 || Integer.parseInt(number) < 1) {
					chanceLabel.setText("超出范围!!!");
					count--;
					messageLabel.setText("您还有" + count + "次机会！！！");
				} else {
					chanceLabel.setText("猜测不正确!!!");
					count--;
					messageLabel.setText("您还有" + count + "次机会！！！");
				}
			}
