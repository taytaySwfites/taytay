...
import cn.edu.jsu.cq.tool.UserBeanTableModel;

public class BeanRankDemo {
	private JFrame frame;
	private JPanel contentPane;// 定义窗体内容面板，放置各组件
	private JTable table;

	public BeanRankDemo(String userName) {
		frame = new JFrame();
		...
		// 将所有记录读出
		List<UserBean> list = ConnectService.getAll("select username,bean from user order by bean desc limit 100");
		// 创建StudentTableModel，传入内容数据集合
		UserBeanTableModel tableModel = new UserBeanTableModel(list);
		...
		table.setFont(new Font("楷体", Font.PLAIN, 22));
		 ...
		 * 标题JLabel
		 */
		JLabel titleLabel = new JLabel("- 豆子排行榜 -");
	 ...
	 * 返回100以内排名
	 */
	private int rankInHundred(List<UserBean> list, String userName) {
