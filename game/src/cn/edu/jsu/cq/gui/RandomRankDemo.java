...
public class RandomRankDemo {
	...
	public RandomRankDemo(String userName) {
		frame = new JFrame();
		...
		// 将所有记录读出
	    List<Record> list = ConnectService.getAllRecord("select * from record_random order by score desc, scoretime asc limit 100");
		 ...
		 * 标题JLabel
		 */
		JLabel titleLabel = new JLabel("- 随机排行榜 -");
		...
		frame.setTitle("随机模式积分排行榜"); // 设置窗口标题
